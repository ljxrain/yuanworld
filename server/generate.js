...
const upload = multer({
    storage,
    limits: {
        fileSize: parseInt(process.env.MAX_FILE_SIZE, 10) || 50 * 1024 * 1024  // 提升到50MB
    },
        ...
        try {
            ...
            if (req.files) {
                if (req.files.image && req.files.image[0]) {
                    console.log(`  - 粉丝照片: ${(req.files.image[0].size / 1024 / 1024).toFixed(2)}MB`);
                }
                if (req.files.idolImage && req.files.idolImage[0]) {
                    console.log(`  - 偶像照片: ${(req.files.idolImage[0].size / 1024 / 1024).toFixed(2)}MB`);
                }
            }
